...
try {
  ...
  let sync: Awaited<ReturnType<CookieAdsProvider["syncReadOnly"]>>;
  try {
    sync = await provider.syncReadOnly({
    ...
    store.updateProviderStatus(account.id, "cookie", "failed", `创建结果核验同步异常：${safeMessage(cause)}`);
    store.setAccountExecutionMode(account.id, "manual-approval", "创建结果核验同步失败，已自动熔断写入。");
    throw cause;
  }
  ...
  if (sync.result.quality.status !== "healthy") {
    store.setAccountExecutionMode(
      account.id,
      "manual-approval",
      `创建结果核验同步质量为 ${sync.result.quality.status}，已自动熔断写入。`,
    );
    throw new Error(
      `创建结果核验数据质量为 ${sync.result.quality.status}，禁止宣布验收成功。`,
    );
  }
  if (sync.result.warnings.length > 0) {
    store.setAccountExecutionMode(account.id, "manual-approval", `创建结果核验数据不完整：${sync.result.warnings.join("；").slice(0, 500)}`);
  }
  ...
  if (missing.length > 0) {
    store.setAccountExecutionMode(account.id, "manual-approval", "创建结果未回读到完整层级，已自动熔断写入。");
    throw new Error(`尚未回读到：${missing.map((item) => item.entityType).join("、")}。`);
  }
  const unsafe = verification.filter((item) => item.statuses.some((status) => status !== "disabled"));
  if (unsafe.length > 0) {
    store.setAccountExecutionMode(account.id, "manual-approval", "核验对象未确认全部关闭，已自动熔断写入。");
    throw new Error(`以下层级未确认全部关闭：${unsafe.map((item) => item.entityType).join("、")}。`);
  }
...
function safeMessage(cause: unknown): string {
  return cause instanceof Error ? cause.message : "未知 Provider 错误";
}
