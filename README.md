...
安装包：`apps/desktop/release/TK-Ads-Automation-Setup-1.4.25.exe`
...
- 只读列表请求失败后会自动重试一次。此前广告层级的派生请求一失败就放弃，整轮同步被判为 partial（`ad` 计为 0 条），账户在界面上显示「读取：未就绪 / 待完善」，直到下一轮同步才恢复。这不只是显示问题：删除和自动复制都要求最近一次同步为 healthy，partial 期间会被连带跳过。生产实测该类失败集中在客户端重启、重新授权后的密集同步窗口，且只打广告条数最多的账户（92 条的账户 24 小时内 13 次，2 条的账户 0 次），特征像限流，日常基线约 0.5%。列表请求没有副作用，重试不存在重复写入风险。
- 派生请求失败的真正原因会写进同步警告。此前只留一句「自动补全请求失败」，39 条历史失败记录里查不出是限流、超时还是会话失效。现在会带上 HTTP 状态码或底层网络错误码。
