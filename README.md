...
安装包：`apps/desktop/release/TK-Ads-Automation-Setup-1.4.20.exe`
...
升级到 1.4.20 后会出现两个预期内的变化，不是故障：

- 所有已接入账户会提示“Provider 能力契约已更新，请重新检测连接”。本版新增了系列级复制能力，重新检测一次即可恢复，系列复制也在检测通过后才开放。
- 首次启动会自动补齐数据库结构（新增系列日预算字段与系列复制幂等表）。升级前保存的广告预设按广告组预算处理，行为不变，无需手工迁移。
- 修复了一个状态写入 bug：若某个广告组的启停 cURL 导入时选中的正好是它自己，此前会被误判为「模板缺少可替换字段」而永久拒绝自动启停，只影响该特定广告组。升级后该广告组的自动启停会恢复正常，无需重新导入 cURL。
- 系列复制新增「创建时间」三选一：关闭（默认）/ 立即投放 / 定时投放；此前定时投放虽然后端已支持，但界面上没有对应入口。
