...
安装包：`apps/desktop/release/TK-Ads-Automation-Setup-1.4.22.exe`
...
- 系列复制在发布前会强制刷新一次账户真实状态，再做命名去重。此前命名去重只读后台定时轮询的本地快照（最长可能滞后 5 分钟），账户里刚创建、还没被下一轮同步捕获到的系列/广告组，可能被误判为「名字可用」而重新分配出去，发布时被 TikTok 判定重名并拒绝；现在会先刷新再规划名字，这次强制刷新失败时直接阻止整个复制请求，不用旧数据冒险撞名。
