...
`packages/core/src/publish-draft.ts` 与 `CookieAdsProvider.publishExistingDrafts`。这份文档
保留为事实底稿。

入口有三个，同一条链路：

- **轮询自动补发布**（2026-09-03 起，主路径）。每轮同步后对账，判成 `draft-only` 就自动把
  发布这一步补上。`draft-only` 这个结论本身已经证明那批组没有被建成正式对象——补发一次不会
  建出重复的组，而这正是它敢自动跑的全部依据。失败计次，试满 3 次停手（见
  `DRAFT_PUBLISH_MAX_ATTEMPTS`），把最后一次的原因留在记录上，红条继续挂着交人工。
- 扩组记录里「结果未知」那一行的「发布草稿」按钮（自动重试试满之后的人工兜底）。
- 系列复制那一侧的同名按钮。**多一个前置条件：系列本身必须已经建出来了。** 系列复制失败若停在
  发布之前，后台只剩一个「草稿系列」（`campaign_id` 为空、只有 `campaign_sketch_id`），发布它
  要连系列一起建，是另一条**没有抓包依据**的链路，代码里不猜，直接说清楚让人去后台处理。
...
**~~一律发成暂停（`is_status_disabled: true`）。~~ 2026-09-03 改成一律发成投放
（`is_status_disabled: false`）。** 原来的理由是「原始扩组选的是立即投放还是先关着没有记录
在案，悄悄开起来烧钱比让人多点一次开关严重得多」。用下来这个取舍是反的：扩组本来就是为了
投，发成暂停等于把「机器没做完的最后一步」换成「人必须记得去后台开一遍」，而那一步一忘就是
一批建好却不投的组躺在后台——比多花一点钱更贵。

连带必须补一刀：**把发出来的广告也开起来**。草稿是 `ad_snap/copy` 克隆的，里面的广告继承了
源广告的开关状态；组开着而广告是关的，整组照样投不出去。扩组发布路径早就有这一步
（`enableCreatedCreatives`），草稿发布此前没有。

另外，创意归属那条坑按「拿不准就不发」处理：`sketch/creative/list` 能给出完整归属就按归属
分创意；给不出，只有当 `save_by_sketch` 自己证明该系列下就这一个广告组草稿时才继续，否则
停手让人去后台手动发。

**部分成功要能收口（2026-09-03）。** 一次扩 3 个组，终态回来「广告组 2/3」是常见结果：2 个
成了正式组、1 个停在草稿。早先要求每个名字都能唯一对上一个草稿，于是整批被拒，那 1 个永远
发不出去——而它恰恰是唯一还需要处理的。现在调用方可以通过 `publishedNames` 声明哪几个已经
建成，发布时跳过它们。

**「已经建成」必须是证明，不能是推断。** 这份名单的作用是允许跳过一次创建，判错就等于漏建
一个组。判据三条缺一不可：在同一个系列下（只对名字会把别的系列的同名组算进来）、状态不是
`ad_create`（草稿也带名字、也在广告组列表里）、系列 ID 读得出来。证不出来的一律进 `missing`
并停手——「不在草稿里」本身证明不了任何事，可能已被人手动发布，也可能已被删除，把后者当成
「已经建成」就会悄悄收口，那批组永远没人认领。判据在 core 的 `resolveDraftPublishTargets`。
...
同日把清理链路也在真机上跑了一遍：三个账户共 8 条够格的草稿，`ad_sketch/delete` 全部
code 0。

**紧接着回读，有 5 条还在**——和发布那边一模一样的最终一致性。约一分钟后再查，8 条全没了，
连查三轮都是 0。所以 **`ad_sketch/delete` 的 code 0 可信，不要加「删完立刻回读确认」**，
那只会稳定地报出根本不存在的失败。

保护期在真实数据上确实拦住了东西：`早餐機_新-0826-060`（0.9 小时前刚失败的那条）被保住，
其余 19–407 小时的才被删。

草稿列表这个「最终一致」的性质，是这两条链路共同的坑：**任何时候都不能拿刚发出写请求之后
的那一次列表查询当判据。**
