...
本手册覆盖 Windows 桌面版安装与升级、Cookie 会话接入、TikTok Marketing API 接入、双 Provider 切换、自动化开关、配置管理、阈值配置与只读同步。

> 本文件由 `packages/manual/guide.json` 自动生成，请勿直接编辑。

## 1. Windows 桌面版安装与升级

1.0.0 是本地优先的 Windows x64 桌面程序，不依赖自建服务器。界面、本地 API、数据库和加密凭据库均在本机运行。

1. 双击 TK-Ads-Automation-Setup-1.0.0.exe，按安装向导完成当前 Windows 用户安装。
