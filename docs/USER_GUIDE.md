...
1.1.0 是本地优先的 Windows x64 桌面程序，不依赖自建服务器。界面、本地 API、调度器、数据库和加密凭据库均在本机运行。

1. 双击 TK-Ads-Automation-Setup-1.1.0.exe，按安装向导完成当前 Windows 用户安装；已安装 1.0.0 的用户可直接覆盖升级。
...
12. 如需真实启停，在 TikTok 页面切换一次测试对象的开关，复制该 update/status 请求的 cURL，仍粘贴到同一个导入区域。程序会识别层级和当前动作，并自动生成开启、关闭两个模板。
13. 只有快速导入无法识别时，才展开高级手动接入填写各字段。

注意事项：

- Cookie 是登录凭据，只能用于本人或明确授权的账户，不要通过聊天、邮件或公开文档传递。
- 列表和状态 cURL 使用同一个入口；每个对象层级只需导入一条真实状态请求，程序会在明确识别状态字段后生成双向模板。
...
6. 确认规则正确后再开启自动执行，并同时开启对应层级的状态管理能力。
