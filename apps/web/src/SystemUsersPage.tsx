...
export function SystemUsersPage({ onError }: { onError: (message: string | null) => void }) {
  ...
  return (
    <section className="page-stack">
      ...
      {canManage && <div className="panel table-panel"><div className="panel-heading"><div><span className="panel-icon"><UserCog size={18} /></span><div><h2>本机账户</h2><p>开发者拥有全部权限；管理员可管理系统；操作员可管理广告；只读用户不能修改数据。</p></div></div><button className="primary-button" onClick={() => setShowCreate(true)} type="button"><Plus size={16} /> 新增账户</button></div><div className="table-wrap"><table><thead><tr><th>账户</th><th>角色</th><th>状态</th><th>最后登录</th><th>创建时间</th><th>操作</th></tr></thead><tbody>{users.map((user) => { const mustKeepDeveloper = user.enabled && user.role === "developer" && enabledDeveloperCount === 1; const cannotChange = user.id === auth.status.user?.id || mustKeepDeveloper; return <tr key={user.id}><td><strong>{user.displayName}</strong><br /><small>{user.username}</small></td><td><select aria-label={`${user.displayName} 的角色`} className="inline-select" disabled={cannotChange} value={user.role} onChange={(event) => void update(user, { role: event.target.value as LocalUserRole })}>{roleOptions(auth.status.user?.role === "developer")}</select></td><td><span className={user.enabled ? "status active" : "status danger"}>{user.enabled ? "已启用" : "已停用"}</span></td><td>{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString() : "从未"}</td><td>{new Date(user.createdAt).toLocaleDateString()}</td><td><button aria-label={`${user.displayName}${user.enabled ? "停用" : "启用"}`} className="secondary-button compact-button" disabled={cannotChange} title={mustKeepDeveloper ? "至少保留一个开发者账户" : undefined} onClick={() => void update(user, { enabled: !user.enabled })} type="button">{user.enabled ? "停用" : "启用"}</button>{mustKeepDeveloper && <small className="inline-protection-note">至少保留一个开发者</small>}</td></tr>; })}</tbody></table></div></div>}
