...
export function AccountManagement(props: Props) {
  ...
  return <section className="p-accounts" aria-label="账户管理">
    ...
    <div className="p-account-surface">
      <div className="p-filters"><label className="p-search"><MagnifyingGlass size={18} /><input aria-label="搜索账户" placeholder="搜索账户名称或 ID" value={query} onChange={(e) => setQuery(e.target.value)} /></label><label>账户状态<select aria-label="账户状态筛选" value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">全部状态</option><option value="healthy">健康</option><option value="warning">待完善</option><option value="danger">异常</option></select></label><label>平台<select aria-label="平台筛选" value={platform} onChange={(e) => setPlatform(e.target.value)}><option value="all">全部平台</option><option value="tiktok">TikTok Ads</option><option value="meta">Meta Ads</option></select></label><label>自动化<select aria-label="自动化范围筛选" value={scope} onChange={(e) => setScope(e.target.value)}><option value="all">全部账户</option><option value="enabled">开关已开启</option><option value="disabled">开关已关闭</option></select></label></div>
    ...
    {detail && (() => {
      ...
      return <Drawer title="账户详情" onClose={() => setDetailId(null)}>
        <div className="p-detail-title"><span className="p-detail-avatar"><UsersThree size={24} /></span><div><h3>{detail.displayName}</h3><small className="p-number">{detail.id}</small><span>{detail.platform === "meta" ? "Meta Ads" : "TikTok Ads"} · {accountTypeNames[detail.accountType]} · 最近同步 {timestamp(state?.latestSync?.finishedAt)}</span></div><Badge tone={health.tone}>{health.label}</Badge></div>
        ...
        <h3>账户信息</h3>
        <dl className="p-detail-list"><dt>平台</dt><dd>{detail.platform === "meta" ? "Meta Ads" : "TikTok Ads"}</dd><dt>账户类型</dt><dd>{accountTypeNames[detail.accountType]}</dd><dt>账户状态</dt><dd><Badge tone={health.tone}>{health.label}</Badge></dd><dt>接入方式</dt><dd>{providerNames[detail.providerKind]}</dd><dt>账户时区</dt><dd>{detail.timezone}</dd></dl>
        <h3>账户能力</h3>
        <ControlRail steps={(() => {
          ...
          const reconnect = { label: "重新接入", onClick: () => { setDetailId(null); props.onConnect(detail); }, disabled: !canManage || disabled || detail.providerKind === "meta-offline" };
        ...
        <div className="p-detail-actions"><div className="p-action-group"><Button tone="primary" disabled={!canManage || disabled} onClick={() => { setDetailId(null); props.onEdit(detail); }}><PencilSimple size={16} />账户设置</Button><Button disabled={!canManage || disabled || detail.providerKind === "meta-offline"} onClick={() => { setDetailId(null); props.onConnect(detail); }}><Plug size={16} />重新接入</Button></div><div className="p-action-group p-action-group-danger"><Button tone="danger" disabled={!canManage || disabled} onClick={() => { setDetailId(null); void run(() => props.onDelete(detail)); }}><Trash size={16} />删除账户</Button></div></div>
