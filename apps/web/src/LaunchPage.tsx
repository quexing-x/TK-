...
export function LaunchPage({ accounts, preferredAccountId, onError }: { accounts: AccountConfig[]; preferredAccountId: string; onError: (message: string | null) => void }) {
  ...
  const targets = useMemo(
    () => accounts.filter((account) => connections[account.id]?.status === "ready"),
    [accounts, connections],
  );
  const visibleTargets = useMemo(() => {
    const query = accountQuery.trim().toLocaleLowerCase();
    return query
      ? targets.filter((account) => account.displayName.toLocaleLowerCase().includes(query))
      : targets;
  }, [accountQuery, targets]);
  ...
  const publishBlockers = [
    ...
    !templateReadiness?.ready ? "创建预设尚未填写完整，请补全预算、出价、地区等必填项。" : null,
    !sheet ? "请导入创建信息表。" : null,
  ...
  useEffect(() => {
    ...
    setSourceAccountId((current) =>
      targets.some((account) => account.id === current)
        ? current
        : targets.find((account) => account.id === preferredAccountId)?.id ?? targets[0]?.id ?? "",
    );
    setTargetIds((current) => current.filter((id) => targets.some((account) => account.id === id)));
  }, [preferredAccountId, targets]);
  ...
  return <section className="page-stack">
    <div className="panel launch-hero"><span><Rocket size={28} /></span><div><span className="eyebrow">多账户投放</span><h2>预设统一配置，表格只填系列、广告组、视频和产品 URL</h2><p>预算、出价、地区与创建时间等由广告预设统一管理；广告名称自动按 YYMMDD:XXX 编号。</p></div><span className={templateReadiness?.ready ? "status active" : "status warning"}>{templateReadiness?.ready ? "创建预设完整" : "创建预设待补全"}</span></div>
    ...
    <div className="panel launch-scope-panel"><div className="panel-heading"><div><span className="panel-icon"><Rocket size={18} /></span><div><h2>发布账户</h2><p>{launchMode === "single" ? "选择一个账户，本批表格将在该账户中从零创建。" : "选择多个账户，系统将在每个账户中分别新建相同结构的广告。"}</p></div></div></div><div className="form-grid">
      {targets.length === 0 ? <p className="field-error">暂无可用账户。请先在“用户管理”完成账户接入，并等待后台连接检测通过。</p> : <>
        {launchMode === "single" && <label className="field"><span>创建账户</span><select value={sourceAccountId} onChange={(event) => setSourceAccountId(event.target.value)}><option value="">请选择</option>{targets.map((account) => <option key={account.id} value={account.id}>{account.displayName}</option>)}</select><small>仅显示数据读取与广告启停均已接入的账户。</small></label>}
        {launchMode === "multi" && <div className="field wide target-account-selector"><span>发布账户（可多选）</span><div className="target-account-toolbar"><input aria-label="搜索可用发布账户" placeholder="搜索已接入账户" value={accountQuery} onChange={(event) => setAccountQuery(event.target.value)} /><span>已选 {targetIds.length} / {targets.length}</span><button className="secondary-button compact-button" onClick={() => setTargetIds(visibleTargets.map((account) => account.id))} type="button">全选当前结果</button><button className="secondary-button compact-button" onClick={() => setTargetIds([])} type="button">清空</button></div><div className="target-account-grid">{visibleTargets.length === 0 ? <p className="target-account-empty">没有匹配的可用账户。</p> : visibleTargets.map((account) => <label key={account.id}><input checked={targetIds.includes(account.id)} onChange={(event) => setTargetIds((current) => event.target.checked ? [...new Set([...current, account.id])] : current.filter((id) => id !== account.id))} type="checkbox" /><span>{account.displayName}</span><small>已接入 · {account.providerKind === "cookie" ? "Cookie" : "Marketing API"}</small></label>)}</div></div>}
      </>}
    </div></div>

    <div className="panel launch-readiness-panel"><div className="panel-heading"><div><span className="panel-icon"><CheckCircle2 size={18} /></span><div><h2>创建检查</h2><p>导入两条 cURL 建立账户会话后，软件使用内置创建协议；再完成广告预设和表格即可发布。</p></div></div><span className={templateReadiness?.ready ? "status active" : "status warning"}>{templateReadiness?.ready ? "可创建" : "预设待补全"}</span></div><div className="sheet-rule-grid">
      ...
      <article><strong>创建功能</strong><span>{!templateReadiness ? "请选择广告预设后检查创建功能。" : templateReadiness.ready ? "内置创建协议和当前预设均已就绪。" : "内置创建协议无需配置；请补全当前广告预设。"}</span></article>
    ...
    <div className="panel launch-sheet-panel"><div className="panel-heading"><div><span className="panel-icon"><FileSpreadsheet size={18} /></span><div><h2>导入创建信息</h2><p>表格仅保留推广系列名称、广告组名称、视频代码和产品 URL。广告名称自动生成。</p></div></div><button className="secondary-button" onClick={() => void downloadLaunchTemplate().catch((cause) => onError(messageOf(cause)))} type="button"><Download size={16} /> 下载模板</button></div>
      ...
      <div className="form-actions"><button aria-describedby={publishBlockers.length > 0 ? "publish-blockers" : undefined} className="primary-button" disabled={busy || !canSave} title={publishBlockers[0]} onClick={() => void savePlan()} type="button">{launchMode === "single" ? "创建并发布" : "向所选账户发布"}{sheet?.rows.length ? `（${sheet.rows.length} 条 × ${selectedAccountIds.length} 个账户）` : ""}</button></div>
    </div>

    <div className="panel table-panel"><div className="panel-heading"><div><span className="panel-icon"><Rocket size={18} /></span><div><h2>投放结果</h2><p>仅保留每次发布计划的账户级结果、成功数量和错误摘要，不展示全部广告。</p></div></div></div><div className="table-wrap"><table><thead><tr><th>创建内容</th><th>预设</th><th>计划任务</th><th>账户结果</th><th>发布结果</th><th>操作</th></tr></thead><tbody>{plans.length === 0 ? <tr><td colSpan={6}>暂无投放计划。</td></tr> : plans.map((plan) => { const created = plan.executionResults.reduce((total, item) => total + item.createdCount, 0); const failed = plan.executionResults.reduce((total, item) => total + item.failedCount, 0); return <tr key={plan.id}><td>{plan.sourceAdName}</td><td>{plan.presetName}</td><td>{plan.launchRows.length} 条 × {plan.targetAccountIds.length} 个账户</td><td>{plan.executionResults.length > 0 ? <small className="plan-execution-summary">成功创建 {created} · 失败 {failed}</small> : "尚未执行"}</td><td><span className={`status ${plan.status === "completed" ? "active" : plan.status === "cancelled" ? "danger" : "warning"}`}>{plan.status === "completed" ? "已发布" : plan.status === "blocked" ? "执行失败" : plan.status}</span>{plan.executionResults.filter((item) => !item.ok).map((item) => <small className="plan-execution-error" key={item.accountId}>{item.accountId}：{item.message ?? "创建失败"}</small>)}</td><td>{["blocked", "draft"].includes(plan.status) && <button disabled={busy} onClick={() => void cancelPlan(plan.id)} type="button"><Trash2 size={14} /> 取消</button>}</td></tr>; })}</tbody></table></div></div>
  </section>;
}
