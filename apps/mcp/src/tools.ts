 ...
 * 工具集与传输层解耦。
 *
 * 同一个工具集要挂到两种宿主上：出 stdio 的本地进程（Codex、Claude 桌面端、DSH、
 * WorkBuddy），和出 HTTP 的本地端点（豆包工作的连接器不认 command，只认 URL）。
 * 两边必须是**同一套工具、同一套提示语**——分成两份实现，早晚会出现「界面里能扩组、
 * 但豆包那边少了个冲突预检」这种只在某一侧复现的问题。
 *
 * 因此这里只负责「有哪些工具」，谁把它接到什么传输上是 server.ts / http.ts 的事。
 */
...
/**
 * 服务器自述。刻意不点名任何具体客户端：同一份说明会被 Codex、Claude、
 * DSH、WorkBuddy、豆包工作读到，写死其中一家的名字，其余几家就会看到一段
 * 讲别人家怎么用的前言。
 */
...
/**
 * 建一个装好全部工具的 MCP server 实例。
 *
 * 每次调用都新建实例、各自持有自己的 LocalApiClient：HTTP 侧的会话是按请求并发来的，
 * 共用一个实例会把不同会话的响应串在一起。客户端本身很轻（只是拿令牌 + fetch 的封装），
 * 每会话一个的代价可以忽略。
 */
export function createServer(api: LocalApiClient = new LocalApiClient()): McpServer {
  ...
  server.registerTool("expand_ad_groups", {
    ...
    const sources = await resolveExpandSources(api, input.accountId, input.sourceAdGroupIds);
